/** Full demo of MP3 Controls, after uploading, open your Serial Monitor and
 * enter commands.
 *
...
void loop() {
  ...
  if (Serial.available()) {
    ...
    case 'e': {
      ...
      case 'J':
        Serial.println("Jazz");
        mp3.setEqualizer(MP3_EQ_JAZZ);
        break;
      case 'C':
        Serial.println("Classic");
        mp3.setEqualizer(MP3_EQ_CLASSIC);
        break;
      case 'B':
        Serial.println("Bass");
    ...
    case 'l': {
      ...
      case 'A':
        ...
        break; // Plays the tracks one after another and repeats
      case 'F':
        Serial.println("Folder");
        mp3.setLoopMode(MP3_LOOP_FOLDER);
        break; // Loop within folder
      case 'O':
        Serial.println("One (repeat playing same file)");
        mp3.setLoopMode(MP3_LOOP_ONE);
        break; // | These seem to do the same, repeat the same track over and
               // over
      case 'R':
        Serial.println("??? - Don't know what it means exactly, in the "
                       "datasheet it is \"RAM\"");
        mp3.setLoopMode(MP3_LOOP_RAM);
        break; //-
      case 'N':
      case 'S':
        Serial.println("None (play file and stop)");
        mp3.setLoopMode(MP3_LOOP_ONE_STOP);
        break; // Default, plays track and stops
      }
    }
      return;
    ...
    case 's': {
      ...
      switch (b) {
      case 'S':
        Serial.println("SD Card (if available).");
        mp3.setSource(MP3_SRC_SDCARD);
        break;
      case 'B':
        Serial.println("on board memory.");
    ...
    case 'F': {
      ...
      fnum = max(1U, min(fnum, 999U));
      folnum = max(1U, min(folnum, 99U));
...
void statusAndHelpOutput() {
  ...
  case MP3_STATUS_STOPPED:
    Serial.println(F("Stopped"));
    break;
  case MP3_STATUS_PLAYING:
    Serial.println(F("Playing"));
    break;
  case MP3_STATUS_PAUSED:
  ...
  case MP3_LOOP_ALL:
    Serial.println(F("Play all tracks, then repeat."));
    break;
  case MP3_LOOP_FOLDER:
    Serial.println(F("Play all tracks in folder, then repeat."));
    break;
  case MP3_LOOP_ONE:
    Serial.println(F("Play one track then repeat (loop track)."));
    break;
  case MP3_LOOP_RAM:
    Serial.println(
        F("Unknown function exactly, seems to play one track then repeat?"));
    break;
  case MP3_LOOP_ONE_STOP:
    Serial.println(F("Play one track then stop."));
    break;
  }
  Serial.println();
  ...
  Serial.println(F("p Play\t\t> Next\t\t< Prev\n[space] Pause\tr Restart from "
                   "start of file\n] Next folder\t[ Prev folder\n"));

  Serial.println(F(
      "f[1-65534]      Play file by (FAT table) index "
      "number\nF[01-99]/[001-999].mp3 Play [001-999].mp3 in folder [01-99]\n"));

  Serial.println(
      F("+ Vol up\t- Vol down\tm Mute\nv[0-30] Set volume\n\ne[N/P/R/J/C/B] "
        "Equalizer (N)ormal, (P)op, (R)ock, (J)azz, (C)lassic, "
        "(B)ass\nl[A/F/O/R/N]   Loop (A)ll, (F)older, (O)ne, (R)???, (N)o "
